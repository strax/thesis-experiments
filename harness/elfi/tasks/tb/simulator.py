# Source: https://github.com/jlintusaari/tb-model/blob/a527b3c5c94b651a5dbf0c2ce2dc65c07b3ac9b6/code/simulator.py
...
class Simulator:
    ...
    def _add_death(self, compliant):
        ...
        observe = (self.time >= self.t_warmup) and (
            self.random_state.rand() <= self.p_observed
        )
        if observe:
            self.n[2] += 1
        ...
        i = _choose_cluster(
            self.clusters[compliant], self.n[compliant] + 1, self.random_state
        )
        self.clusters[compliant, i] -= 1
...
def analytical_means(
    burden, trans, death, trans_non_c, death_non_c, p_compliance=0.95, p_observed=0.80
):
    """Return the balance population sizes and the approximated mean number of observations in one year
...
def test_simulator():
    ...
    s = Simulator(200, 1, 6, trans_non_c=1.1, death_non_c=0.52, t_obs=2, t_warmup=10)
    while s.next_event():
        pass
